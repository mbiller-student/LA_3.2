...
std::string RemoveFirstSubstring(std::string s1, std::string s2)
{
	...
	std::size_t substring_found_index = s1.find(s2); // size_t needed
	if (substring_found_index != std::string::npos){
		s1.erase(substring_found_index, s2.length());
	} 
	return s1;
}
...
{
	if (original <= 0) {throw std::invalid_argument("Input must be greater than 0");} // invalid if input is <= 0
	while(original % 2 == 0) {
		original = original / 2;
	}
	return original;
...
std::vector<int> MultiplesFilter(std::vector<int> v, int divides_by) // added `v` to pass parameter into function body
{
	std::vector<int> non_divisible;

	if (divides_by == 0) {throw std::invalid_argument("Input cannot be 0");}
	
	for(std::size_t i = 0; i < v.size(); i++) {
		if (v[i] % divides_by != 0) {
			non_divisible.push_back(v[i]);
		}
	}
...
std::vector<bool> EvenMask(std::vector<int> v) // added `v` to pass parameter into function body 
{
